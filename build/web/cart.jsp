...
<html lang="en">
    ...
    <body style="color: #e5e7eb">
                    ...
                    <section class="cart__about__property">
                        ...
                        <c:forEach items="${sessionScope.cart}" var="entry">
                            <div class="cart__about__property-desc row">
                                <!-- img -->
                                <div class="cart__about__property-img col-sm-6 col-md-3 " style="overflow: auto; height: 150px;">
                                    <a href="DetailFoodServlet?fid=${entry.key.foodId}" class="cart__about__property-img-link">
                                        <c:if test="${entry.key.typeToBuy.equals('Food')}">                           
                                            <img src="${entry.key.image}" alt="">
                                        </c:if>
                                        <c:if test="${entry.key.typeToBuy.equals('Ingredient')}">
                                            <c:forEach items="${entry.key.listingredients}" var="ingrImg">
                                                <img src="${ingrImg.ingredientImg}" alt="" >
                                            </c:forEach>
                                        </c:if>
                                    </a>
                                </div>
                                    ...
                                    <div class="row cart__about__property-infor-row ">
                                        ...
                                        <div class="cart__about__property-infor-desc col-md-6">
                                            <div class="cart__about__property-name">
                                                <h3 style="text-transform: capitalize;">${entry.key.name}</h3>
                                            </div>
                                            <div class="cart__about__property-quantity ">
                                                <h3>Quantity: <span>${entry.value}</span></h3>
                                            </div>
                                            <div class="cart__about__property-quantity ">
                                                <h3>Type: <span>${entry.key.typeToBuy}</span></h3>
                                            </div>
                                            <div class="cart__about__property-update-delete">
                                                <button type="button" class="cart__property-update"
                                                        data-id="${entry.key.foodId}" 
                                                        data-name="${entry.key.name}"
                                                        data-img="${entry.key.image}"
                                                        data-quantity="${entry.value}"
                                                        data-type="${entry.key.typeToBuy}">Edit</button>    

                                                <!-- delete -->
                                                                                                
                                                    
                                                    <button class="cart__property-delete" name="btnactioneditCart" value="remove"
                                                            data-id="${entry.key.foodId}" 
                                                        data-name="${entry.key.name}"
                                                        data-img="${entry.key.image}"
                                                        data-quantity="${entry.value}"
                                                        data-type="${entry.key.typeToBuy}">
                                                        Delete 
                                                        <div class="icon-1">
                                                            <svg
                                        ...
                                        <div class="cart__about__property-price col-md-3">                                                   
                                            <h3 class="property-price-cart">${ PriceFood != null ? PriceFood : PriceIngrs }</h3>

                                        </div>
                                        <!-- total -->
                                        <div class="cart__about__property-total col-md-3">
                                            <c:set var="TotalProduct" value="${PriceFood != null ? PriceFood*entry.value + TotalProduct : PriceIngrs*entry.value + TotalProduct }" ></c:set>
                                            <h3 class="property-total-cart">${PriceFood != null ? PriceFood*entry.value : PriceIngrs*entry.value }</h3>                                            
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="cart__about__check-subtotal-container">
                            <div class="row cart__about__check-subtotal-container-row">
                                <div class="check-subtotal-quantity-total-item col-md-4">
                                    <h3>Subtotal ( <span>${CountProduct} Items</span> ):</h3>
                                </div>
                                        ...
                                        <div class="modal__product-body-update-quantity">
                                            <form action="EditProductCartServlet" method="post">
                                                ...
                                                <div class="product__detail-form-desc">
                                                    <h1>Quantity</h1>
                                                    <div class="cart__about__property-quantity ">
                                                        <h3>Type: <span class="typeProductclick"></span></h3>
                                                    </div>
                        ...
                        <div class="  modal-confirm">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <p>Do you really want to delete <span class="nameProductDelete" style="background-color: #000; color: #fff; padding: 4px; border-radius: 4px"></span> ? This process cannot be undone.</p>
                                </div>
                        ...
                        <div class="container card-empty">
                            <div class="row">
                                <div class="col-12 desc-cart-empty">
                                    <!-- Image -->
                                    <img src="./assets/img/slider/cartempty.svg" class="h-200 h-md-300 mb-3" alt="">
                                    <!-- Subtitle -->
        ...
        <script>
            ...
            const modalProductImage = document.querySelector('.modal__product-body-img img');
            const modalProductName = document.querySelector('.modal__product-body-name');
            const modalQuantityInput = document.querySelector('#quantity-input');

//            1 nut update id va type
            const modalProductIdUpdate = document.querySelector('.takeIdUpdate');
            const modalProductTypeToUpdate = document.querySelector('.tackTypeUpdate');

            const modalTypeInput = document.querySelector('.typeProductclick');
            for (const openeditPr of openEditProducts) {
                openeditPr.addEventListener('click', function () {
                    const foodData = {
                        idF: this.getAttribute('data-id'),
                        nameF: this.getAttribute('data-name'),
                        imageF: this.getAttribute('data-img'),
                        quantityF: this.getAttribute('data-quantity'),
                        typeF: this.getAttribute('data-type'),
                        listIng: this.getAttribute('data-listI')
                    };
                    modalProductName.innerText = foodData.nameF;
                    modalProductImage.src = foodData.imageF;
                    modalQuantityInput.value = foodData.quantityF;
                    modalTypeInput.innerText = foodData.typeF;
                    modalProductIdUpdate.value = foodData.idF;
                    modalProductTypeToUpdate.value = foodData.typeF;
        ...
        const nameFoodDelete = document.querySelector('.nameProductDelete');
        const modalIdDelete = document.querySelector('.takeIdDelete');
        const modalTypeDelet = document.querySelector('.taketypeTodelete');
        ...
        for (const openConfirm of btnDelete) {
            openConfirm.addEventListener('click', function () {               
                ...
                nameFoodDelete.innerText = foodData.nameF;
               modalIdDelete.value = foodData.idF;
                 modalTypeDelet.value = foodData.typeF;
